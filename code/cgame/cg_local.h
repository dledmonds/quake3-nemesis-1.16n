...
// client debugging used by cg_debugNemesis
#define NMS_DEBUG_PREFIX	"^3NMS: ^7"
#define NMS_DEBUG_STATS		0x00000001	// debug statistics
#define NMS_DEBUG_SNAP		0x00000002	// debug snapshot
#define NMS_DEBUG_MODEL		0x00000004	// debug enemy models

// special effects
#define NMS_SE_SPARKS		0x00000001	// sparks
#define NMS_SE_PLASMA		0x00000002	// plasma trail effect

// chat fltering
#define CHAT_NO_PUBLIC_CHAT		0x00000001
#define CHAT_NO_TEAM_CHAT		0x00000002
#define CHAT_NO_SERVER_CHAT		0x00000004
...
typedef struct {
	...
	int				breathPuffTime;  // Nemesis - breathPuffs
...
typedef struct {
	team_t team;  // to work out when to reset stats
...
typedef struct {
	...
	int			clientNum;  // Nemesis - enemyModel

	vec4_t		xhairColor;

	vec4_t		enemyLegsColor;
	vec4_t		enemyTorsoColor;
	vec4_t		enemyHeadColor;
	vec4_t		enemyRailColor;
	vec4_t		enemyRocketColor;
	vec4_t		enemyPlasmaColor;
	vec4_t		enemyGrenadeColor;

	vec4_t		teamRailColor;
	vec4_t		teamRocketColor;
	vec4_t		teamPlasmaColor;
	vec4_t		teamGrenadeColor;

	qboolean	takenScreenshot;	// auto screenshot
	int			screenshotTime;		// auto screenshot

	qboolean	musicStart;
	int			musicStartTime;
 
	qboolean	persistentFollow;		// used for persistent following
	int			persistentFollowId;		// used for persistent following
	int			persistentFollowTime;	// used for persistent following

	qboolean	playerRecord;			// used by recordplayer command
	...
	// drowning bubbles
	qboolean	drowning[MAX_CLIENTS];
	int			drowningTime[MAX_CLIENTS];
	...
	refdef_t			*refdef_current;		// Handling of some drawing el
#endif
...
typedef struct {
	...
	qhandle_t	threewaveCharsetShader;
	qhandle_t	cpmaCharsetShader;
	...
	qhandle_t	healthSphere;
	qhandle_t	medalAccuracy;
	qhandle_t	railRingsShader2;
	qhandle_t	railStartShader;
	qhandle_t	railEndShader;
	...
	// Nemesis
	qhandle_t	plasmaBallShader2;
	qhandle_t	teamSparks;
	// End
	...
	// Nemesis
	qhandle_t	plasmaExplosionShader2;
	// End
	...
	sfxHandle_t	win;
	sfxHandle_t	lose;
	sfxHandle_t	winMusic;
	sfxHandle_t	iceShatter;
	...
	sfxHandle_t hitSound[4]; // Promode hit tones
...
typedef struct {
	...
	int				maxclients;
	int				fastRail;
	int				freezelimit;			// Nemesis - DA Freeze
	int				g_gameMod;				// Nemesis - NoGhost freeze detection
	char			modName[MAX_QPATH];		// Nemesis - Mod name detection
...
extern	vmCvar_t		cg_specEnemyModel;
extern	vmCvar_t		cg_drawAcc;
extern	vmCvar_t		cg_useTeamIcons;
extern	vmCvar_t		cg_drawSpeed;
extern	vmCvar_t		cg_drawSpeedBar;
extern	vmCvar_t		cg_enableBreath;
extern	vmCvar_t		cg_scoreboardType;
extern	vmCvar_t		cg_hudType;
extern	vmCvar_t		cg_hudFile;
extern	vmCvar_t		cg_hudDebug;
extern	vmCvar_t		cg_autoScreenshot;
extern	vmCvar_t		cg_autoRecord;
extern	vmCvar_t		cg_thawSoundType;
extern	vmCvar_t		cg_teamInfoType;
extern	vmCvar_t		cg_iceShells;
extern	vmCvar_t		cg_noChatBeep;
extern	vmCvar_t		cg_autoAnnounce;
extern	vmCvar_t		cg_noFriendShader;
extern	vmCvar_t		cg_noConnectionShader;
extern	vmCvar_t		cg_noProjectileTrail;
extern	vmCvar_t		cg_scorePlum;
extern	vmCvar_t		cg_debugNemesis;
extern	vmCvar_t		cg_teamColorWeapons;
extern	vmCvar_t		cg_specialEffects;
extern	vmCvar_t		cg_ammoCheck;
extern	vmCvar_t		cg_chatTime;
extern	vmCvar_t		cg_chatHeight;
extern	vmCvar_t		cg_chatFilter;
extern	vmCvar_t		cg_consoleFilter;

extern	char killedby[MAX_QPATH];

// Nemesis - Ported from ET source by OSP :), used for crosshair & enemy color  
...
//void	CG_DrawIntermissionInfo( void );
float	CG_DrawAccIntermission( float y );
void	CG_ProModeHUD( void );
float	CG_DrawProModeScores( float y );
void	CG_WeaponAccCheck( void );
char	*CG_DrawWeaponAcc( float y, int weapon );
...
void CG_DrawBlinkingString( int x, int y, int style, vec4_t color, float alpha, char *string, int fontWidth, int fontHeight );
void CG_DrawBlinkingRect( int x, int y, int style, vec4_t color, float alpha, int width, int height );
...
void CG_DrawHeightGauge( int x, int y, int width, int height, vec4_t color, int value, qboolean reverse );
...
void CG_DrawStringExtComplex(int x, int y, const char *string, const float *setColor, 
		qboolean forceColor, qboolean shadow, int charWidth, int charHeight,
		int maxChars, qhandle_t shader);
...
int CG_DrawAlignedField(int x, int y, int width, int chrWidth, int chrHeight, int value, qboolean leftIndent, qboolean leftAlign);
void CG_DrawStatusBarFlag( float x, int team );
void CG_Draw3DModel( float x, float y, float w, float h, qhandle_t model, qhandle_t skin,
...
void CG_mvDraw(int pID, int position);
...
void CG_BodyObituary( entityState_t *ent, char *targetName );	// Nemesis - Freeze
