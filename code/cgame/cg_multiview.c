...
//	CG_mvUpdateClientInfo(pID);
	...
	row = position / 2;
	col = position % 2;
	// possible of 4 290 x 210 windows with 10 pixel padding on each
	rd_x = 25 + (col * 10) + (col * 290);
	rd_y = 25 + (row * 10) + (row * 210);
	rd_w = 290;
	rd_h = 210;
...
//	} else {
		...
		refdef.vieworg[2] += DEFAULT_VIEWHEIGHT;
//	}
...
//	}
	...
	CG_DrawBorder(b_x, b_y, b_w, b_h, 1, colorYellow);

	if (1) {
		int tmp;
		clientInfo_t *ci = &cgs.clientinfo[cg.snap->ps.clientNum];

		tmp = CG_DrawStrlen(ci->name);
		CG_DrawStringExt(b_x + b_w/2 - (tmp*8/2), b_y + 1, ci->name, colorWhite, qfalse, qtrue, 8, 8, 20);

		trap_R_SetColor(colorYellow);
		tmp = cg.snap->ps.stats[STAT_HEALTH];
		CG_DrawField(b_x + b_w/2 - 48, b_y + b_h - 12, 3, 10, 12, tmp, qtrue);

		tmp = cg.snap->ps.stats[STAT_ARMOR];
		CG_DrawField(b_x + b_w/2 + 48, b_y + b_h - 12, 3, 10, 12, tmp, qtrue);
	}
