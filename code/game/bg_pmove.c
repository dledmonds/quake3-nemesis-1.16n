...
// need to define weapon speed defaults for people using the client only
int	InstaGib;
int	MGSpeed = 100;
int	SGSpeed = 1000;
int	GLSpeed = 800;
...
static void PM_Weapon( void ) {
	...
	if ( pm->ps->ammo[ pm->ps->weapon ] != -1 ) {
		// Infinite ammo allowed for instagib mode
		if ( InstaGib ) {
	...
	case WP_RAILGUN:
		addTime = RGSpeed;
		pm->ps->stats[STAT_RAILTIME] = RGSpeed;
		break;
	case WP_BFG:
		addTime = BFGSpeed;
		break;
