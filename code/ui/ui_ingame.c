...
#define	NEMESIS_INGAME			20
#define	NEMESIS_SUPERHUD		21
...
typedef struct {
	...
	menutext_s		team;
	menutext_s		setup;
	menutext_s		nemesis_setup;
	menutext_s		nemesis_superHud;
	menutext_s		server;
	menutext_s		leave;
	menutext_s		restart;
	menutext_s		addbots;
...
void InGame_Event( void *ptr, int notification ) {
	...
	case ID_RESTART:
		UI_ConfirmMenu( "RESTART ARENA?", (voidfunc_f)0, InGame_RestartAction );	// Nemesis - compiler warning fix
		break;

	case ID_QUIT:
		UI_ConfirmMenu( "EXIT GAME?", (voidfunc_f)0, InGame_QuitAction );	// Nemesis - compiler warning fix
		break;
...
void InGame_MenuInit( void ) {
	...
	y += INGAME_MENU_VERTICAL_SPACING;
	s_ingame.nemesis_setup.generic.type			= MTYPE_PTEXT;
	s_ingame.nemesis_setup.generic.flags		= QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_ingame.nemesis_setup.generic.x			= 320;
	s_ingame.nemesis_setup.generic.y			= y;
	s_ingame.nemesis_setup.generic.id			= NEMESIS_INGAME;
	s_ingame.nemesis_setup.generic.callback		= InGame_Event; 
	s_ingame.nemesis_setup.string				= "NEMESIS";
	s_ingame.nemesis_setup.color				= color_red;
	s_ingame.nemesis_setup.style				= UI_CENTER|UI_SMALLFONT;

	y += INGAME_MENU_VERTICAL_SPACING;
	s_ingame.nemesis_superHud.generic.type		= MTYPE_PTEXT;
	s_ingame.nemesis_superHud.generic.flags		= QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	s_ingame.nemesis_superHud.generic.x			= 320;
	s_ingame.nemesis_superHud.generic.y			= y;
	s_ingame.nemesis_superHud.generic.id		= NEMESIS_SUPERHUD;
	s_ingame.nemesis_superHud.generic.callback	= InGame_Event; 
	s_ingame.nemesis_superHud.string			= "NEMESIS HUD";
	s_ingame.nemesis_superHud.color				= color_red;
	s_ingame.nemesis_superHud.style				= UI_CENTER|UI_SMALLFONT;
	...
	Menu_AddItem( &s_ingame.menu, &s_ingame.nemesis_setup );
	Menu_AddItem( &s_ingame.menu, &s_ingame.nemesis_superHud );
...
// Nemesis - dynamic menus (thanks HypoThermia!)
/*
===========================
...
#define DM_ID_JOINRED		100
#define DM_ID_JOINBLUE		101
#define DM_ID_JOINGAME		102
#define DM_ID_SPECTATE		103
...
static char* refereeCommandStrings[] = {
	"ref mute %i", // RC_MUTE
	"ref unmute %i", // RC_UNMUTE
	"ref forceteam %i r", // RC_TEAMRED
	"ref forceteam %i b", // RC_TEAMBLUE
	"ref forceteam %i s", // RC_TEAMSPEC
	"ref warn %i \"Warned by a referee\"", // RC_WARN
	"ref kick %i", // RC_KICK
	"ref kickban %i", // RC_KICKBAM
	0
};
...
/*
=================
DynamicMenu_AddItemExtra
=================
*/
static qboolean DynamicMenu_AddItemExtra( const char* string, 
	int id, createHandler crh, eventHandler evh, int extraInfo)
{
	int index, depth;
	...
	s_dynamic.data[index].createSubMenu = crh;
	s_dynamic.data[index].runEvent = evh;
	s_dynamic.data[index].extraInfo = extraInfo;
...
static void DynamicMenu_FinishSubMenuInit( void )
{
	...
	scale = 0.60f;//UI_ProportionalSizeScale(UI_SMALLFONT);
	if (submenu)
	{
...
static void DynamicMenu_MenuItemDraw( void* self )
{
	...
	UI_DrawString2( x + 2, y + 2, t->string, colorBlack, 10, 10 ); // Hack for drop-shadow
	UI_DrawString2( x, y, t->string, colorWhite, 10, 10 );
...
=================
*/
static void DM_Nemesis_Submenu( int index )
{
	UI_NemesisMenu();
}

static void DM_Nemesis_SuperHud( int index )
{
	UI_SuperHudMenu();
}
...
=================
*/
static void DM_ServerInfo_SubMenu( int index )
{
	UI_ServerInfoMenu();
}
...
=================
*/
static void DM_AddBot_SubMenu( int index )
{
	UI_AddBotsMenu();
}
...
/*
=================
DM_RemoveBot_SubMenu
=================
*/
static void DM_RemoveBot_SubMenu( int index )
{
	UI_RemoveBotsMenu();
}
...
=================
*/
static void DM_OldMenu_Event( int index )
{
	//UI_PopMenu();  // Cant do this!  for SP it causes bad frame overlaps, thus killing the local server
	UI_InGameMenu();
}
...
=================
*/
static void DM_TeamMenu_Event( int index )
{
	int cmd;
...
=================
*/
static void DM_SetupMenu_Event( int index )
{
	//UI_PopMenu();
	UI_SetupMenu();
}
...
/*
=================
DM_DisconnectMenu_Event
=================
*/
static void DM_DisconnectMenu_Event( int index )
{
	//UI_PopMenu();
	trap_Cmd_ExecuteText( EXEC_APPEND, "disconnect\n" );
}
...
=================
*/
static void DM_QuitMenu_Event( int index )
{
	//UI_PopMenu(); <- Was causing error
	//UI_ConfirmMenu( "EXIT GAME?", NULL, InGame_QuitAction );

	// Ah phug it, bypass the annoying confirmation screen, just quit instantly :p
...
/*
=================
DM_FollowCommand_Event
=================
*/
static void DM_FollowCommand_Event( int index )
{
	int clientId;

	// get the client id
	clientId = s_dynamic.data[index].extraInfo;

	DynamicMenu_Close();
	trap_Cmd_ExecuteText( EXEC_APPEND, va( "follow %i\n", clientId ) );
}


/*
=================
DM_RefereeCommand_Event
=================
*/
static void DM_RefereeCommand_Event( int index )
{
	int depth;
	int clientId, cmd;
	const char* s;
	...
	{
		Com_Printf("RefereeCommand_Event: index %i"
			" at wrong depth (%i)\n", index, depth);
		DynamicMenu_Close();
		return;
	}
	...
	case RC_KICKBAN:
		break;
	default:
		Com_Printf("RefereeCommand_Event: unknown command (%i)\n", cmd);
		DynamicMenu_Close();
		return;
	};
	...
	s = va( refereeCommandStrings[ cmd ], clientId );	
	// issue the command
	DynamicMenu_Close();
	...
	createHandler crh, eventHandler evh )
{
	...
	for( n = 0; n < numPlayers; n++ ) {
		...
		DynamicMenu_AddItemExtra(name, 0, crh, evh, n);
	}
}
...
static void DM_TeamList_SubMenu( void ) 
{
	...
	// add option depending on game type
	...
	case GT_FFA:
	case GT_TOURNAMENT:
		DynamicMenu_AddItem( "GAME", DM_ID_JOINGAME, (voidfunc_f)0, DM_TeamMenu_Event );
		break;

	default:
	case GT_TEAM:
	case GT_CTF:
		DynamicMenu_AddItem( "RED", DM_ID_JOINRED, (voidfunc_f)0, DM_TeamMenu_Event );
		DynamicMenu_AddItem( "BLUE", DM_ID_JOINBLUE, (voidfunc_f)0, DM_TeamMenu_Event );
		break;
	}

	DynamicMenu_AddItem( "SPECTATOR", DM_ID_SPECTATE, (voidfunc_f)0, DM_TeamMenu_Event );
...
/*
=================
DM_FollowPlayerList_SubMenu
=================
*/
static void DM_FollowPlayerList_SubMenu( void ) 
{
	DynamicMenu_SubMenuInit();

	DynamicMenu_AddListOfPlayers( 0, (voidfunc_f)0, DM_FollowCommand_Event );
...
/*
=================
DM_RefereePlayerList_SubMenu
=================
*/
static void DM_RefereePlayerList_SubMenu( void ) 
{
	DynamicMenu_SubMenuInit();

	DynamicMenu_AddListOfPlayers( 0, (voidfunc_f)0, DM_RefereeCommand_Event );
...
/*
=================
DM_RefereeCommandList_SubMenu
=================
*/
static void DM_RefereeCommandList_SubMenu( void ) 
{
	DynamicMenu_SubMenuInit();

	DynamicMenu_AddItem( "Mute", RC_MUTE, DM_RefereePlayerList_SubMenu, (intfunc_f)0 );
	DynamicMenu_AddItem( "Unmute", RC_UNMUTE, DM_RefereePlayerList_SubMenu, (intfunc_f)0);
	DynamicMenu_AddItem( "Team Red", RC_TEAMRED, DM_RefereePlayerList_SubMenu, (intfunc_f)0);
	DynamicMenu_AddItem( "Team Blue", RC_TEAMBLUE, DM_RefereePlayerList_SubMenu, (intfunc_f)0);
	DynamicMenu_AddItem( "Team Spectator", RC_TEAMSPEC, DM_RefereePlayerList_SubMenu, (intfunc_f)0);
	DynamicMenu_AddItem( "Warn", RC_WARN, DM_RefereePlayerList_SubMenu, (intfunc_f)0);
	DynamicMenu_AddItem( "Kick", RC_KICK, DM_RefereePlayerList_SubMenu, (intfunc_f)0);
	DynamicMenu_AddItem( "Ban", RC_KICKBAN, DM_RefereePlayerList_SubMenu, (intfunc_f)0);
...
static void DynamicMenu_InitPrimaryMenu( void )
{
	...
	DynamicMenu_AddItem( "TEAM", 0, DM_TeamList_SubMenu, (intfunc_f)0 );
	DynamicMenu_AddItem( "NEMESIS", 0, (voidfunc_f)0, DM_Nemesis_Submenu );
	DynamicMenu_AddItem( "NEMESIS HUD", 0, (voidfunc_f)0, DM_Nemesis_SuperHud );
	DynamicMenu_AddItem( "SETUP", 0, (voidfunc_f)0, DM_SetupMenu_Event );
	DynamicMenu_AddItem( "FOLLOW", 0, DM_FollowPlayerList_SubMenu, (intfunc_f)0 );
	DynamicMenu_AddItem( "ADD BOTS", 0, (voidfunc_f)0, DM_AddBot_SubMenu );
	DynamicMenu_AddItem( "REMOVE BOTS", 0, (voidfunc_f)0, DM_RemoveBot_SubMenu );
	DynamicMenu_AddItem( "SERVERINFO", 0, (voidfunc_f)0, DM_ServerInfo_SubMenu );

	// make sure it is noghost mod or cannot use ref commands
	trap_GetConfigString( CS_SERVERINFO, sysInfo, sizeof(sysInfo) );
	if ( !Q_strncmp( Info_ValueForKey( sysInfo, "Mod_Author" ), "Maverick", 8 ) ) {
		DynamicMenu_AddItem( "REFEREE", 0, DM_RefereeCommandList_SubMenu, (intfunc_f)0 );
	}

	DynamicMenu_AddItem( "RESUME", 0, (voidfunc_f)0, DM_Close_Event );
	DynamicMenu_AddItem( "DISCONNECT", 0, (voidfunc_f)0, DM_DisconnectMenu_Event );
	//DynamicMenu_AddItem( "OLD MENU", 0, (voidfunc_f)0, DM_OldMenu_Event );
	DynamicMenu_AddItem( "QUIT", 0, (voidfunc_f)0, DM_QuitMenu_Event );
